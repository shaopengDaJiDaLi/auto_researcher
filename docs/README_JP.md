...
既存のAI研究ツールは論文を**書く**手助けをします。Auto Researcherは実験を**実行**します。
...
python -m auto_researcher.runner --check  # 確認
```
...
# 方法2: コマンドライン
python -m auto_researcher.runner --project . --directive "label smoothing 0.1を試す"
...
はい。Codeはファイルの読み書き・修正が可能。dry-runで検証してから訓練開始。
</details>
...
| コマンド | 機能 |
|---------|------|
| `/auto-research` | 24/7自律THINK→EXECUTE→REFLECT実験ループを起動 |
